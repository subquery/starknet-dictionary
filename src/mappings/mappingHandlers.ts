...
export async function handleBlock(block: StarknetBlock): Promise<void> {
  ...
  const calls = await Promise.all(block.transactions.map(tx => handleTransaction(tx)));
...
export function handleTransaction(transaction: StarknetTransaction): Call[] {
    ...
    switch (transaction.type) {
        ...
        case "INVOKE":
            ...
            calls.push(...handleInvokeL1Handler(transaction));
            break;
...
//Handle INVOKE and L1_HANDLER transaction
function handleInvokeL1Handler(transaction: StarknetTransaction): Call[] {
    const calls: Call[] = [];
